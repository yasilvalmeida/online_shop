var rating_data_table;
$(() => {
    $.post("api/shop.php?action=fetchSingleProduct",
    { 
        id : $("#pid").val()
    },
    (data, status) => {
        if(status == "success"){
            try {
                ...
                $("#title").html("OSCB | Rating of " + r.name);
                $("#header").html("Rating of " + r.name);
    ...
    rating_data_table = $('#dataTable').DataTable({
        "processing": true,
        "serverSide": false,
        "order": [],
        "ajax": {
            url: "api/shop.php?action=fetchAllRating",
            type: "POST",
            data: { t_product_fk: $("#pid").val() }
        },
        "oLanguage": {
...
function removeAsync() {
    ...
    $.post("api/shop.php?action=removeRating",
    { 
        id: $("#id_to_remove").val()
    },
    (data, status) => {
