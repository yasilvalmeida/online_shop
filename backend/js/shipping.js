var shipping_data_table;
$(() => {
    shipping_data_table = $('#dataTable').DataTable({
        "processing": true,
        "serverSide": false,
        "order": [],
        "ajax": {
            url: "api/shop.php?action=fetchAllShipping",
            type: "POST",
            data: { }
        },
        "oLanguage": {
...
function insertAsync() {
    ...
    $.post("api/shop.php?action=insertShipping",
    { 
        ...
        if(status == "success"){
            try {
                ...
                if(parseInt(r.result) != NaN && parseInt(r.result) == 1){
                    ...
                    clear_form();
                    shipping_data_table.ajax.reload();
                }
                else{
...
function update(shipping) {
    ...
    $("#id_to_update").val(shipping.id);
    $("#name_to_update").val(shipping.name);
    $("#price_to_update").val(shipping.price);
    tips.addClass("alert-light");
...
function updateAsync() {
    ...
    tips.addClass("alert-light");
    tips.html("<img src='../img/loader.gif' />");
    $.post("api/shop.php?action=updateShipping",
    { 
        id   : $("#id_to_update").val(),
...
function removeAsync() {
    ...
    tips.addClass("alert-light");
    tips.html("<img src='../img/loader.gif' />");
    $.post("api/shop.php?action=removeShipping",
    { 
        id: $("#id_to_remove").val()
    },
    (data, status) => {
